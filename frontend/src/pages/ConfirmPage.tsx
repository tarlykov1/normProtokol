...
export function ConfirmPage() {
  ...
  return (
    <div className="space-y-3 rounded border bg-white p-4">
      ...
      {summary && <p className="text-sm">Valid: {summary.count_valid}, warnings: {summary.count_warnings}, errors: {summary.count_errors}</p>}
      <div className="flex flex-wrap gap-2">
        ...
        <button className="rounded border px-3 py-1" onClick={async () => { await protocolsApi.generateDocx(data.id); await refetch() }}>Сформировать Word</button>
        <button className="rounded border px-3 py-1" onClick={async () => downloadBlob(await protocolsApi.downloadDocx(data.id), `protocol-${data.id}.docx`)}>Скачать Word</button>
