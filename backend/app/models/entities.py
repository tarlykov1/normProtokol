...
class TaskCandidate(Base):
    ...
    parent_context: Mapped[str | None] = mapped_column(String(255), nullable=True)
    context_label: Mapped[str | None] = mapped_column(String(100), nullable=True)
    assignees_raw: Mapped[str | None] = mapped_column(Text, nullable=True)
    assignees_normalized: Mapped[list[str]] = mapped_column(JSON, default=list)
    assignees_display: Mapped[str | None] = mapped_column(String(500), nullable=True)
    coordinator: Mapped[str | None] = mapped_column(String(255), nullable=True)
    deadline_kind: Mapped[str | None] = mapped_column(String(50), nullable=True)
    ...
    item_kind: Mapped[str] = mapped_column(String(50), default="task")
    discussed_flag: Mapped[bool] = mapped_column(Boolean, default=True)
    skipped_discussion_flag: Mapped[bool] = mapped_column(Boolean, default=False)
